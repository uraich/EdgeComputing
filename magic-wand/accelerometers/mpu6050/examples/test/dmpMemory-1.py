...
# divide in into blocks

dataPos = 0
blocks = []
block_no = 0

if address % MPU6050_DMP_MEMORY_BANK_SIZE:
    block = data[:MPU6050_DMP_MEMORY_BANK_SIZE - (address % MPU6050_DMP_MEMORY_BANK_SIZE)]
    print("Block: {:d}, block length: {:d}".format(block_no, len(block)))
    # print all the full chunks
    print("full chunks:")
    if len(block) >= MPU6050_DMP_MEMORY_CHUNK_SIZE:
        for j in range(0,len(block)-MPU6050_DMP_MEMORY_CHUNK_SIZE,MPU6050_DMP_MEMORY_CHUNK_SIZE):
            #print("0x{:02x} ".format(j),end='')
            chunk = data[dataPos : dataPos + MPU6050_DMP_MEMORY_CHUNK_SIZE]
            dataPos += MPU6050_DMP_MEMORY_CHUNK_SIZE
            for k in range(MPU6050_DMP_MEMORY_CHUNK_SIZE):
                print("0x{:02x} ".format(chunk[k]),end="")
            print("")
    # print the missing bytes
    print("rest")
    if len(block) % MPU6050_DMP_MEMORY_CHUNK_SIZE :
        chunk = block[- (len(block) % MPU6050_DMP_MEMORY_CHUNK_SIZE) :]
        for j in range(len(chunk)):
            print("0x{:02x} ".format(chunk[j]),end="")
        print("")
        dataPos += len(chunk)
    block_no += 1      
    print("block: {:d}, dataPos: {:d}".format(block_no,dataPos))

if len(data) - dataPos  >= MPU6050_DMP_MEMORY_BANK_SIZE:
    for i in range(dataPos,
                   len(data)-MPU6050_DMP_MEMORY_BANK_SIZE,
                   MPU6050_DMP_MEMORY_BANK_SIZE):
        block = data[dataPos:dataPos + MPU6050_DMP_MEMORY_BANK_SIZE]
        print("Block length: {:d}".format(len(block)))
        for j in range(MPU6050_DMP_MEMORY_BANK_SIZE // MPU6050_DMP_MEMORY_CHUNK_SIZE):
            chunk = block[j:(j+1)*MPU6050_DMP_MEMORY_CHUNK_SIZE]
            print("chunk_length: {:d}, dataPos: {:d}".format(len(chunk),dataPos))
...
if (len(data)-dataPos) % MPU6050_DMP_MEMORY_BANK_SIZE:
    block = data[dataPos:len(data)]
    print("block length: {:d}".format(len(block)))
    # print the full blocks
    if len(block) >= MPU6050_DMP_MEMORY_CHUNK_SIZE:
        for j in range(len(block)-MPU6050_DMP_MEMORY_CHUNK_SIZE,MPU6050_DMP_MEMORY_CHUNK_SIZE):
            for k in range(MPU6050_DMP_MEMORY_CHUNK_SIZE):
                print(j+k)
                # print("0x{:02x} ".format(block[j+k]),end="")
            print("")  
    # print the missing bytes
    print("rest")
    for j in range(len(block) % MPU6050_DMP_MEMORY_CHUNK_SIZE):
        print("0x{:02x} ".format(block[-(len(block) % MPU6050_DMP_MEMORY_CHUNK_SIZE):][j]),end="")
    print("")
    dataPos += len(block)
print("dataPos after printing data: {:d}, length of data: {:d}".format(
    dataPos,len(data)))
