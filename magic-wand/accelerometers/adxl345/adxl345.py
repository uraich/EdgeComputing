...
class ADXL345(object):
    ...
    def setXOffset(self,offset):
        if offset < -128 or offset > 127:
            print("Offset must be in the range -128 .. 127")
            return
        tmp = bytearray(1)
        tmp[0] = offset
        if self.debug :
    ...
    def getFullRes(self) :
        return self.getBit(ADXL345_DATA_FORMAT,FULL_RES)
    ...
    def getFIFOMode(self) :
        return self.getBits(ADXL345_FIFO_CTL,FIFO_TYPE,FIFO_TYPE_SIZE)

    # FIFO_STATUS
    def getFIFO_Status(self):
        return self.i2c.readfrom_mem(self.adxl345_addr,ADXL345_FIFO_STATUS,1)[0]

    def fifoDataAvailable(self) :
        return (self.getFIFO_Entries() > 0)
    
    def getFIFO_Entries(self):
        return self.getBits(ADXL345_FIFO_STATUS,FIFO_ENTRIES,FIFO_ENTRIES_SIZE)

    def getFIFO_Trigger(self):
        return self.getBit(ADXL345_FIFO_STATUS,FIFO_TRIG)
    
    def clearFIFO(self):
        # stop the measurements
        measBit = self.getMeasure()
        self.setMeasure(False)
        while self.getFIFO_Entries() != 0 :
            trash = self.i2c.readfrom_mem(self.adxl345_addr,ADXL345_DATAAX0,6)
        if self.debug :
            print("FIFO successfully cleared")
        self.setMeasure(measBit)
        
    # get data from FIFO
    def getFIFO_Data(self) :
        if not self.fifoDataAvailable() :
            return None
        else:
            if self.getOverrunSource() :
                print("Overrun Error")
                self.clearFIFO()
    ...
    # get physical values
    def AccelerometerPhysical(self) :
        # get the range
        range = self.getRange()
        data = self.getAccelerometerData()
        if not self.getFullRes() :
            return (data[0]*range/512,data[1]*range/512,data[2]*range/512) # 10 bits signed value
        else:
            return (data[0]/512,data[1]/512,data[2]/512)
    
    def calibrate(self,x_goal = None, y_goal = None, z_goal = None):
        # get the FIFO mode
        fifoMode = self.getFIFOMode()
        self.setFIFOMode(BYPASS)
        running = self.getMeasure()
        ...
        if z_goal:
            ...
            # z should measure 1g
            ranges = {ACCEL_2G:2,ACCEL_4G: 4, ACCEL_8G: 8,ACCEL_16G: 16}
            if self.getFullRes() :
                self.z_goal = -256
            else :
                if self.debug:
                    print("range: {:d}".format(ranges[self.getRange()]))
            self.z_goal = -512//ranges[self.getRange()]
        if self.debug :
            print("calibrate: x_goal: {:d} y_goal: {:d} z_goal = {:d}".format(
                self.x_goal,self.y_goal,self.z_goal))

        while True:
            cnt = 0    
            self.setXOffset(x_offset)
            self.setYOffset(y_offset)
            self.setZOffset(z_offset)
            if self.getDataReadySource():
                data = self.getAccelerometerData()
                if self.debug:
                ...
                if x_done and y_done and z_done:
                    print("Offset set to: x_offset =  {:d}, y_offset = {:d} z_offset = {:d}".format(x_offset,y_offset,z_offset))
                    # Set back fifo mode
                    self.setFIFOMode(fifoMode)
                    self.setMeasure(False)
                    self.clearFIFO()
                    self.setMeasure(running)
                    break
                if data[0] == self.x_goal:
                    x_done = True
                if not x_done:
                    if data[0] > self.x_goal:
                        x_offset -= 1
                    elif data[0] < self.x_goal:
                        x_offset += 1
                if data[1] == self.y_goal:
                    y_done = True
                if not y_done:
                    if data[1] > self.y_goal:
                        y_offset -= 1
                    elif data[1] < self.y_goal:
                        y_offset += 1
                        
                if data[2] == self.z_goal:
                    z_done = True
                if not z_done:
                    if data[2] > self.z_goal:
                        z_offset -= 1
                    elif data[2] < self.z_goal:
                        z_offset += 1 
            else :
                cnt += 1
                if cnt > 1000:
